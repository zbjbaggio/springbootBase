...
public interface ProductMapper {
    ...
    @Insert("insert into t_product_info(name,title,price,status,operator_id,create_time,description,product_no) values(#{name},#{title},#{price},#{status},#{operatorId},now(),#{description},#{productNo})")
    ...
    @Update("update t_product_info set name = #{name}, title = #{title}, status = #{status},price = #{price}, operator_id = #{operatorId}, description = #{description} where id = #{id}")
